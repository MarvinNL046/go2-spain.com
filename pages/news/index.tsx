...
export default function NewsPage({ articles }: NewsPageProps) {
  ...
  const breadcrumbs = [
    { name: 'Home', href: '/' },
    { name: isNl ? 'Spain Nieuws' : 'Spain News', href: '/news/' },
  ];

  return (
    <>
      <SEOHead
        title={isNl ? "Spain Nieuws — Laatste Updates & Verhalen | Go2Spain" : "Spain News — Latest Updates & Stories | Go2Spain"}
        description={isNl ? "Blijf op de hoogte van het laatste Spain nieuws: toerisme-updates, visumwijzigingen, veiligheidswaarschuwingen en culturele verhalen. Dagelijks bijgewerkt." : "Stay informed with the latest Spain news: tourism updates, visa changes, safety alerts, and cultural stories. Updated daily."}
      />
        ...
        <section className="bg-white shadow-sm">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <h1 className="text-4xl font-bold font-heading text-gray-900 mt-4">
              {isNl ? 'Spain Nieuws' : 'Spain News'}
            </h1>
            <p className="mt-3 text-lg text-gray-600 max-w-3xl">
              {isNl ? 'Het laatste nieuws uit Spain — toerisme-updates, visumwijzigingen, economie, veiligheid en cultuur. Dagelijks bijgewerkt uit betrouwbare bronnen.' : 'The latest news from Spain — tourism updates, visa changes, economy, safety, and culture. Updated daily from trusted sources.'}
            </p>
          </div>
        </section>
